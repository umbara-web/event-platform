...
export const getEventReviews = asyncHandler(async (req: Request, res: Response) => {
  ...
  const result = await reviewService.getEventReviews(req.params.eventId as string, pagination);

  ApiResponse.paginated(res, MESSAGES.REVIEW.FETCHED, result.data, result.pagination);
});
...
export const getEventReviewStats = asyncHandler(async (req: Request, res: Response) => {
  if (!req.params.eventId) {
    throw ApiError.badRequest('Event ID is required');
  }

  const stats = await reviewService.getEventReviewStats(req.params.eventId as string);
